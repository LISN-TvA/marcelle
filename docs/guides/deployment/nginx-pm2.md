...
For this tutorial, we are going to take an example of a Marcelle App (called rlhf-nlp-app in this tutorial) composed of three elements:
 ...
 - A Marcelle Backend (running a mongoDB database) (cf. [adding a backend](/adding-a-backend))
 - A python server delivering a machine learning model through the Ray library (cf. [server side inference](/machine-learning-models/server-side-inference))
...
*Note : the following scripts assume that you have sudo permissions*
...
If your Marcelle app has a backend, you will need MongoDB (*you can skip this section if you don't have one*). The following commands are standard install commands from [the official mongdb website](https://www.mongodb.com/docs/v7.0/tutorial/install-mongodb-on-ubuntu/).
```sh
...
*Note: after installing MongoDB, you will need to import your current data into the newly created MongoDB database. This can be done with [the mongodump and mongostore commands](https://www.mongodb.com/docs/manual/tutorial/backup-and-restore-tools/)*.
...
pnpm build
```
*Note : if when building you get an error "top-level await is not available" you will need update vite.config.js of your project with the following value*
```js
esbuild: {
	supported: {
...
npm install pm2 -g
```
*Note : Because we have installed NodeJS locally, every package installed with NodeJS is also going to be installed locally.*
...
```
*Note: because nginx will install itself in /etc/, it will be accessible to our system user and we don't need to make a local install.*
...
```
*Note: Also make sure to update the code that might contain some "localhost:8000" or "localhost:3030" to actually use the hostname.*
